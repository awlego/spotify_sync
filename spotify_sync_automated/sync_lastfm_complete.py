...
Designed to handle large histories (70k+ scrobbles) reliably
"""
...
from src.core.models import PlayHistory, SyncProgress
...
class CompleteSyncManager:
    ...
    def sync_chunk(self, chunk: dict, progress: SyncProgress):
            ...
            for i in range(0, len(tracks), self.batch_size):
                ...
                with self.db.session_scope() as session:
                    self.db.update_sync_progress(
                        ...
                        last_page=page,
                        total_tracks_synced=progress.total_tracks_synced + tracks_added,
    ...
    def run_sync(self, force_reset: bool = False):
        ...
        with self.db.session_scope() as session:
            ...
            self.db.update_sync_progress(
                ...
                self.sync_type,
                status='running',
                started_at=datetime.utcnow() if not progress.started_at else progress.started_at
            )
        ...
        for i, chunk in enumerate(chunks[start_chunk_idx:], start_chunk_idx):
            try:
                ...
                chunk_added = self.sync_chunk(chunk, progress)
                ...
                with self.db.session_scope() as session:
                    self.db.update_sync_progress(
                        ...
                        total_tracks_synced=progress.total_tracks_synced + chunk_added,
                ...
                logger.info(f"Progress: {i+1}/{len(chunks)} chunks, "
                          f"{progress.total_tracks_synced + chunk_added:,} total tracks, "
