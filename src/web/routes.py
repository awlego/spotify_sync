    ...
    def api_update_playlist(playlist_type):
        ...
        try:
            if playlist_type == 'most_listened':
                success, error_msg = current_app.playlist_service.update_most_listened_playlist()
            elif playlist_type == 'recent_favorites':
                success, error_msg = current_app.playlist_service.update_recent_favorites_playlist()
            elif playlist_type == 'binged_songs':
                success, error_msg = current_app.playlist_service.update_binged_songs_playlist()
            else:
            ...
            if success:
                ...
                return jsonify({'success': False, 'error': error_msg or f'{playlist_type.replace("_", " ").title()} playlist update failed'})
        except Exception as e:
